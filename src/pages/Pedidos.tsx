...
import { CalendarPlus, CakeSlice, ChevronRight, ClipboardList, Search, WalletCards } from 'lucide-react'
...
function Pedidos({ onVerDetalle }: PedidosProps) {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      if (!activo) return
      if (errorSupabase) { console.error(errorSupabase); setError('No se pudo cargar el historial.'); setPedidos([]) }
  ...
  const filtrados = useMemo(() => {
    ...
    return pedidos.filter((pedido) => {
      ...
      const coincideTexto = !texto || [pedido.nombre_cliente, pedido.telefono ?? '', pedido.sabor_torta, pedido.sabor_relleno].some((v) => v.toLowerCase().includes(texto))
  ...
  const kpi = useMemo(() => {
    ...
    return {
      ...
      ventas: activos.reduce((s, p) => s + Number(p.precio_cotizado ?? 0), 0),
      pendientes: activos.filter((p) => p.estado_pedido === 'Pendiente').length,
      porCobrar: activos.reduce((s, p) => s + obtenerPago(p).saldo, 0),
    }
  }, [pedidos])

  return (
    <main className="p-5 md:ml-64 md:p-8 lg:p-10">
      <header className="mb-8"><p className="text-sm text-[#756870]">Historial</p><h2 className="mt-1 text-3xl font-bold text-[#5C3A4D]">Todos los pedidos</h2><p className="mt-2 text-sm text-[#756870]">Consulta encargos anteriores y futuros en una sola pantalla.</p></header>

      <section className="mb-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi etiqueta="Pedidos registrados" valor={cargando ? '...' : kpi.total} /><Kpi etiqueta="Ventas registradas" valor={cargando ? '...' : `$${kpi.ventas.toFixed(2)}`} /><Kpi etiqueta="Por preparar" valor={cargando ? '...' : kpi.pendientes} /><Kpi etiqueta="Saldo por cobrar" valor={cargando ? '...' : `$${kpi.porCobrar.toFixed(2)}`} />
      </section>

      <section className="mb-5 flex flex-col gap-3 rounded-2xl border border-[#EEDDE3] bg-white p-4 sm:flex-row">
        <label className="relative flex-1"><Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9A8B93]"/><input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar cliente, teléfono, torta o relleno..." className="w-full rounded-xl border border-[#E5D7DE] bg-[#FFFDFC] py-3 pl-10 pr-4 outline-none focus:border-[#EC3D7F]"/></label>
      ...
      <section className="space-y-3">
        {filtrados.map((pedido) => {
          ...
          return <article key={pedido.id} className="rounded-2xl border border-[#EEDDE3] bg-white p-4 sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <button type="button" onClick={() => onVerDetalle(pedido.id)} className="flex flex-1 items-start gap-4 text-left">
                ...
                <div><p className="font-bold text-[#5C3A4D]">{pedido.nombre_cliente}</p><p className="mt-1 text-sm text-[#756870]">{fecha} · {formatearHora(pedido.hora_entrega)}</p><p className="mt-1 text-xs text-[#9A8B93]">{pedido.sabor_torta} · {pedido.sabor_relleno} · Chantilly {pedido.chantilly}</p></div>
              </button>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <a href={crearUrlGoogleCalendar(pedido)} target="_blank" rel="noreferrer" className="rounded-xl border border-[#E5D7DE] p-2.5 text-[#5C3A4D] hover:bg-[#FBF1F4]" title="Agregar a Google Calendar"><CalendarPlus size={17}/></a>
